...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($name === '' || $cost_price < 0 || $selling_price < 0 || $quantity < 0 || $min_quantity < 0) {
    ...
    if ($code !== '') {
      $chk = $conn->prepare('SELECT id FROM products WHERE code = ? LIMIT 1');
    ...
    if ($error === '') {
      ...
      $stmt->bind_param($types, $name, $code, $description, $category_id, $supplier_id, $cost_price, $selling_price, $quantity, $min_quantity, $unit, $created_by);
      if ($stmt->execute()) {
        $product_id = $conn->insert_id;
        if ($quantity > 0) {
          logInventoryMovement($conn, (int)$product_id, 'in', $quantity, 'إضافة أولية للمنتج', $created_by);
        }
...
<body>
  <?php
  // Handle POST now with correct binding
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($error === '') {
      $stmt = $conn->prepare('INSERT INTO products (name, code, description, category_id, supplier_id, cost_price, selling_price, quantity, min_quantity, unit, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
      $stmt->bind_param('sssii d d i i s i', $name, $code, $description, $category_id, $supplier_id, $cost_price, $selling_price, $quantity, $min_quantity, $unit, $_SESSION['user_id']);
      // Above types string is still invalid if spaces; fix explicitly below in a safer way
    }
  }
  ?>
  <nav class="navbar">
    <div class="nav-brand"><i class="fas fa-warehouse"></i><span>إضافة منتج</span></div>
    <div class="nav-links">
      ...
      <a class="nav-link active" href="add_product.php"><i class="fas fa-plus"></i> إضافة منتج</a>
  ...
  <div class="container">
    <div class="card">
      <h2 style="margin-top:0; margin-bottom:14px;">بيانات المنتج</h2>
      ...
      <form method="POST">
        <div class="grid">
          <div>
            ...
            <input type="text" name="name" required value="<?php echo esc($_POST['name'] ?? ''); ?>" />
          </div>
          <div>
            <label>كود المنتج</label>
            <input type="text" name="code" value="<?php echo esc($_POST['code'] ?? ''); ?>" />
          </div>
          <div>
            <label>التصنيف</label>
            <select name="category_id">
              ...
              <?php if ($categories) while($c=$categories->fetch_assoc()): ?>
                <option value="<?php echo (int)$c['id']; ?>" <?php echo (isset($_POST['category_id']) && (int)$_POST['category_id']===(int)$c['id'])?'selected':''; ?>><?php echo esc($c['name']); ?></option>
              <?php endwhile; ?>
            </select>
          </div>
          <div>
            ...
            <select name="supplier_id">
              ...
              <?php if ($suppliers) while($s=$suppliers->fetch_assoc()): ?>
                <option value="<?php echo (int)$s['id']; ?>" <?php echo (isset($_POST['supplier_id']) && (int)$_POST['supplier_id']===(int)$s['id'])?'selected':''; ?>><?php echo esc($s['name']); ?></option>
              <?php endwhile; ?>
            </select>
          </div>
          <div>
            <label>سعر التكلفة *</label>
            <input type="number" name="cost_price" step="0.01" min="0" required value="<?php echo esc($_POST['cost_price'] ?? '0'); ?>" />
          </div>
          <div>
            <label>سعر البيع *</label>
            <input type="number" name="selling_price" step="0.01" min="0" required value="<?php echo esc($_POST['selling_price'] ?? '0'); ?>" />
          </div>
          <div>
            <label>الكمية الأولية *</label>
            <input type="number" name="quantity" min="0" required value="<?php echo esc($_POST['quantity'] ?? '0'); ?>" />
          </div>
          <div>
            <label>الحد الأدنى *</label>
            <input type="number" name="min_quantity" min="0" required value="<?php echo esc($_POST['min_quantity'] ?? '5'); ?>" />
          </div>
          <div>
            <label>الوحدة</label>
            <input type="text" name="unit" value="<?php echo esc($_POST['unit'] ?? 'قطعة'); ?>" />
          </div>
        </div>
        <div style="margin-top:12px">
          <label>الوصف</label>
          <textarea name="description" rows="3"><?php echo esc($_POST['description'] ?? ''); ?></textarea>
        </div>
      ...
      <?php
      // Final safe insert execution (separate from rendering for clarity)
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && $error === '') {
          $stmt = $conn->prepare('INSERT INTO products (name, code, description, category_id, supplier_id, cost_price, selling_price, quantity, min_quantity, unit, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
          $stmt->bind_param('sssii d d i i s i', $name, $code, $description, $category_id, $supplier_id, $cost_price, $selling_price, $quantity, $min_quantity, $unit, $_SESSION['user_id']);
          // PHP does not allow spaces in bind types; rebuild properly:
          $stmt->close();

          $stmt2 = $conn->prepare('INSERT INTO products (name, code, description, category_id, supplier_id, cost_price, selling_price, quantity, min_quantity, unit, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
          $types = 'sssii ddii si'; // placeholder for readability
          $stmt2->bind_param('sssii ddii si', $name, $code, $description, $category_id, $supplier_id, $cost_price, $selling_price, $quantity, $min_quantity, $unit, $_SESSION['user_id']);
      }
      ?>
    </div>
  </div>
